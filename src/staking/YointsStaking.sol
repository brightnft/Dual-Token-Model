...
contract YointsStaking {
  ...
  uint256 public lockupDuration = 2 minutes; // Will be 30 days in production
  ...
  constructor(IERC20 _yointsToken, IERC20 _yapesToken) {
  ...
  function stake(uint256 _amount) external /*nonReentrant*/ {
    ...
    if (userStake.amount > 0) {
      uint256 pending = ((userStake.amount * accYapesPerShare) / 1e12) - userStake.rewardDebt;
      if (pending > 0) {
  ...
  function claimReward() external /*nonReentrant*/ {
    ...
    require(pending <= contractBalance, "Insufficient balance in contract for rewards");
    if (pending > 0) {
      yapesToken.safeTransfer(msg.sender, pending);
      userStake.rewardDebt = (userStake.amount * accYapesPerShare) / 1e12;

...
contract Yapes is ERC20, Ownable, ReentrancyGuard {
  ...
  modifier tradeControl() {
    require(
      _msgSender() == owner() || (activeTrading && !_blacklisted[_msgSender()]),
  ...
  function blockAccount(address account) external onlyOwner {
    require(block.timestamp <= deploymentTime + 72 hours, "Blacklisting period expired");
  ...
  function claimOverflow(address tokenAddress, uint256 tokenAmount) external onlyOwner nonReentrant {
  ...
  function transfer(address recipient, uint256 amount) public override tradeControl returns (bool) {
  ...
  /// @return A boolean that indicates if the operation was successful.
  function transferFrom(address sender, address recipient, uint256 amount) public override tradeControl returns (bool) {
